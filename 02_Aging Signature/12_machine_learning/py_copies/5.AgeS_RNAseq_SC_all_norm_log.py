...
#%%
f_in="Kirshner_2t_merged.csv"
# for this set you need other marks for young and old
df_SC=pd.read_csv(f_in, sep="\t", index_col=0)
...
#%%  choose either of 3 available data sets below
f_in="GSE100426_SC_Mann_corr.tsv"
#%%
f_in="Grover_count_filteredcorr.txt"
#%%
#data from Kowalczyk
f_in="GSE59114_C57BL6_merged.csv" 
#%% read and check all other sets than Kirshner
df_SC=pd.read_csv(f_in, sep="\t", index_col=0)
cells=df_SC.columns #these are samples names
#find samples and indexes for young and old, age is for color etc
youngID, oldID, age=[],[],[]
...
plt.title("present to total as is")
...
plt.scatter(pc1[:thresh],pc2[:thresh], c=scores[:thresh], cmap="Purples", edgecolor="black")
plt.clim(0,1)
plt.colorbar()    
plt.xlim(-35,35)
plt.ylim(-25,25)
